...
static bool process_variable(t_minishell *minishell, char **result, unsigned int *pos)
{
	char *var_name = extract_var_name(minishell->user_input, pos);
    if (!var_name)
	return false;
	
    char *var_value = ft_getenv(minishell->envp, var_name);
    if (var_value) {
		char *new_result = ft_strjoin_gnl(*result, var_value);
        if (!new_result) {
			free(var_name);
            return false;
        }
